...
volatile Encoder Enc[2] = {{2,3,0}, {5,6,0}};
 ...
 *
 */
void EncoderPiero_Final_Outputs_wrapper(int32_T *pos,
			const real_T *xD)
{
 ...
 *
 */
void EncoderPiero_Final_Update_wrapper(int32_T *pos,
			real_T *xD)
{
...
if (xD[0]!=1) {
    ...
    # ifndef MATLAB_MEX_FILE
       ...
       /* Enc[0].pinA=pinA[0];      /* set pin A              */
        /* Enc[0].pinB=pinB[0];      /* set pin B              */
        /* Enc[1].pinA=pinA[0];      /* set pin A              */
         /*Enc[1].pinB=pinB[0];      /* set pin B              */
    
        /* set encoder pins as inputs                            */
        pinMode(Enc[0].pinA, INPUT); 
        pinMode(Enc[0].pinB, INPUT); 
        pinMode(Enc[1].pinA, INPUT); 
        pinMode(Enc[1].pinB, INPUT); /* Nuestra variable se llama Enc */
